...
console.log('checksExistsUserAccount')
...
function checksTodoExists(request, response, next) {
  if(!request.headers.username || !request.params.id){
    return response.status(404).json({ error: "Username or todo id undefined"})
  }

  if(!validate(request.params.id)){
    return response.status(400).json({ error: "Todo id required"})
  }
